...
var createdBy = document.querySelector('#createdBy');  //# para id y . para clases

createdBy.innerHTML = 'Aplicación creada por Ivan';
...
// Ayuda: usar 'this' en el constructor adad
...
//    1) Crear un elemento 'div' y asignárselo a una variable denominada 'toDoShell' (se crea con document.crateElement)
//    2) Asignarle a 'toDoShell' la clase 'toDoShell' (se pasa con ClassName)
//    3) Crear un elemento 'span' y asignárselo a una variable denominada 'toDoText' (se crea con document.crateElement)
...
//    7) Agregar 'toDoText' como hijo de 'toDoShell' (se agraga con .appendChild (document.))
...
function buildToDo(todo, index) {
  ...
  toDoText.appendChild (document.toDoShell);
}
...
function buildToDos(toDos) {
  // Tu código acá:
  var newToDos = toDos.map(buildToDos);
  return newToDos;
...
function displayToDos() {
  ...
  var arrayToDos = buildToDo (toDoItems);
...
function addToDo() {
  // Tu código acá:
 var toDoImput = document.getElementById ('toDoInput');
 var newToDo = new ToDo (toDoImput.value);
...
//   1) Seleccionar el elemento cuyo id es 'addButton' (se seleccionna con .getelementsById)
